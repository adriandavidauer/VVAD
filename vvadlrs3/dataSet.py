...
# Remove Sample as it is replaced by "Featured" Sample
    ...
    def getSamples(self, path, featureType, shape, dryRun=False, relative=True):
        ...
        if not dryRun:
            ...
            while success and sampleConfigList:
                # KZ Use env for additional debug/develop log
...
def transformToFeatures(path, shapeModelPath=None, shape=None):
    ...
    faceFeaturesSample = FeaturedSample()
    ...
    lipFeaturesSample = FeaturedSample()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--shape", help="[x,y] defines the size to which face or lip images will be resized - this is the input "
                              "size of the net", type=list)
    parser.add_argument(
